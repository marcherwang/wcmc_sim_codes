%Function plotAvgSensingAccuracy: calculate the average sensing accuracy from DB
%Parameters: bitRate; k_max, the max ratio of radius of interferer from the
%origin to the link distance
%Returned Value: null.
function plotAvgSensingAccuracy(bitRate, k_max, standardName)
close all;
constants;
%set the topology
...
for cstIter=1:length(csThreshes) %different cs thresholds
    for fadingIter=1:length(fadingModels) %different fading models
        ...
        if fadingModels(fadingIter)==NONFADING
            curs= exec(conn, ['select avg(idleSuccProb+busyFailProb) from wifi_test_tbl where bit_rate='  num2str(bitRate)  ' and fading='  num2str(fadingModel)  ' and cs_thresh=' num2str(cst)  ' and r_i >=0 and r_i<='  num2str(maxRi)]);
            curs = fetch(curs);
            data=curs.Data;
        ...
        elseif fadingModels(fadingIter)==NAKAGAMI
            ...
            for nakaIter=1:length(nakagamiMs) %different m's for Nakagami fading 
                ...
                curs= exec(conn, ['select avg(idleSuccProb+busyFailProb) from wifi_test_tbl where bit_rate='  num2str(bitRate)  ' and fading='  num2str(fadingModel)    ' and nakagami_m='  num2str(nakaM)  ' and cs_thresh=' num2str(cst)  ' and r_i >=0 and r_i<='  num2str(maxRi)]);
                curs = fetch(curs);
                data=curs.Data;
